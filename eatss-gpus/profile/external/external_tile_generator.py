...
def find_base_tile_sizes(target, strategy='default'):
    cmd = ["ppcg", "--target=cuda", "--dump-sizes",
           "external/%s/%s.c" % (target, target), "-I", "polybench-c-3.2/utilities/"]
    if strategy != 'default':
        cmd.insert(3, "--load-schedule")
        cmd.insert(4,  "custom_schedules/%s-max-fusion-schedule.txt" % (target))
...
def get_thread_adj_block_size(num_dims, tile_config=[], scaling_factor=0.5):
    ...
    for i in range(num_dims):
        ...
        if i > 0 and _tile_config[i] > _max_thread_count:
            _tile_config[i] = int(_max_thread_count)
            _max_thread_count //= _tile_config[i]
        elif i > 0 and _tile_config[i] <= _max_thread_count:
            _max_thread_count //= _tile_config[i]
...
def get_max_shared_mem_per_sm(cap_shared_mem=1.0, compute='nvidia-smi'):
    shared_mem_lookup = {'nvidia-smi': 196608,
                         'tegra': 131072, 'mid-tier': 98304}
    if compute is None:
        device = cuda.Device(0)
        attrs = device.get_attributes()
        max_shared_mem = attrs[pycuda.driver.device_attribute.MAX_SHARED_MEMORY_PER_MULTIPROCESSOR]
    else:
        max_shared_mem = int(shared_mem_lookup[compute])
    max_shared_mem *= cap_shared_mem
    if cap_shared_mem < 1.0:
        logger.info("capping shared memory to: %d" % int(max_shared_mem))
    else:
        logger.info("using full shared memory: %d" % int(max_shared_mem))
...
# constant foot print range


def cfp_tile_range(target, base, num_dims, strategy='mult_one_at_a_time', restart=None, express=False, platform='a100', warp_frac=1.0):
    idx = 1
    ...
    if strategy == 'mult_one_at_a_time':
        ...
        i_idx, j_idx, k_idx, m_idx = 0, 0, 0, 0
        if num_dims == 4:
            tile_list_ = [4, 8, 10, 16, 32, 48, 50, 64]
            n = len(tile_list_)
            if restart is not None:
                i_idx, j_idx, k_idx, m_idx = [
                    tile_list_.index(x) for x in restart]
                m_idx = (m_idx + 1) % n
            logger.info("resuming at: %sx%sx%s" % (tile_list_[
                  i_idx], tile_list_[j_idx], tile_list_[k_idx], tile_list_[m_idx]))
                    ...
                    for j in range(j_idx, n):
                        if restart is not None and j == j_idx:
                            for k in range(k_idx, n):
                                if restart is not None and k == k_idx:
                                    for m in range(m_idx, n):
                                        yield [tile_list_[i], tile_list_[j], tile_list_[k], tile_list_[m]]
                                else:
                                    for m in range(0, n):
                                        yield [tile_list_[i], tile_list_[j], tile_list_[k], tile_list_[m]]
        ...
        if num_dims == 3:
            ...
            if restart is not None:
                ...
                i_idx = (i_idx + 1) % n
            logger.info("resuming at: ", tile_list_[i_idx])
            for i in range(i_idx, n):
                yield [tile_list_[i]]
    elif strategy == 'load_from_file':
        config_dir = './oracle'
        df = pd.read_csv('%s/%s-heavy.tiles' %
        ...
        config_dir = './oracle'
        df = pd.read_csv('%s/%s-ppcg-external.tiles' %
                         (config_dir, platform), header=None, sep=':')
        df.columns = ['TILE_CONF', 'TARGET', 'CAP', 'WARP_FRAC', 'TILE_CONFIG']
        df = df[df['TARGET'] == target]
        cap_arr = df['CAP'].to_numpy()
...
if __name__ == '__main__':
    # get_gpu_stats()
    config_dir = './oracle'
    df = pd.read_csv('%s/iteartive/%s-heavy.tiles' %
                     (config_dir, 'a100'), header=None, sep=':')
    df.columns = ['SUMMARY', 'TARGET', 'ITR',
                  'CAP', 'WARP_FRAC', 'TILE_CONFIG']
    logger.info(df['TARGET'].unique())

    # for t in cfp_tile_range('2mm', 32, 3, strategy='load_from_file', restart=None, express=False):
    # logger.info(t)
